...
// Необходимо реализовать автодополнение. 
// При введении в инпут первых букв страны - появляется список стран, 
// которые начинаются на те буквы, которые вы ввели. 
// Чем больше букв вы вводите - тем меньше стран в списке. 
// Страну можно выбрать, кликнув по ней мышкой - в этом случае ее название появится в инпуте.
...
inpCountry.addEventListener('input', function() {
    let inpVal = inpCountry.value.trim().toLowerCase(),
        inpLength = inpCountry.value.trim().length;
    list.innerHTML = '';
    res = countries.filter(function(str) {
        return (inpLength && inpVal == str.trim().toLowerCase().substr(0, inpLength));
    });
...
list.addEventListener('click', function(e) {
    if (e.target && e.target.tagName == 'P') {
        inpCountry.value = e.target.textContent.trim();
        list.innerHTML = '';
    }
});